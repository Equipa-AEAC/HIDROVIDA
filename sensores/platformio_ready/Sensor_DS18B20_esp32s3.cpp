...
namespace {
constexpr uint8_t ONE_WIRE_BUS_PIN = 7;
...
void setup() {
  ...
  Serial.println();
  Serial.println("[BOOT] ESP32-S3 + DS18B20 test starting...");
  ...
  if (sensorCount == 0) {
    Serial.println("[WARN] No DS18B20 detected. Check wiring, 4.7k pull-up, power, and ground.");
    return;
  }
