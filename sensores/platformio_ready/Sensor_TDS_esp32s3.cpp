...
constexpr uint8_t TDS_SAMPLES = 10;
...
void setup() {
  ...
  Serial.println();
  Serial.println("[BOOT] ESP32-S3 Gravity TDS test starting...");
  ...
  Serial.println("[INFO] Serial calibration commands: enter, cal:707, exit");
}
...
void loop() {
  ...
  const float waterTemperatureC = DEFAULT_TEMPERATURE_C;
  const float tdsPpm = readAveragedTds(waterTemperatureC, TDS_SAMPLES);
  ...
  if (tdsPpm <= 1.0f) {
    const uint16_t rawAvg = readAverageRawAdc(TDS_PIN, DEBUG_ADC_SAMPLES);
    Serial.printf("[WARN] Very low TDS (%.0f ppm). ADC(avg)=%u. Check probe in water, sensor power, and shared GND.\n",
                  tdsPpm,
                  rawAvg);
    if (rawAvg < ADC_LOW_THRESHOLD) {
      Serial.println("[DEBUG] ADC is near zero. Sensor output may be disconnected.");
    }
  }
